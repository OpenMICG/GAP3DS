...
class EgoEvalDataset(data.Dataset):
    def __init__(self, config, train=False):
        ...
        self.train_random_ori_list = [0]  
        # We've put in quite a bit of effort to augment the GIMO dataset, and you have the freedom to adjust the ori_aug settings according to your needs
        # Just a heads-up: the longer the self.train_random_ori_list, the more time the process will take...

        self.dataset_info = pd.read_csv(os.path.join(self.dataroot, 'dataset_with_motion_label.csv'))
        self.parse_data_info()

        # If this is the first time, preprocess raw dataset, this would take ~3min (depend on the fps).
        ...
        self.global_joint_max = np.max(np.array(torch.cat([self.joints_input, self.joints_label], dim=1)))
        self.global_joint_min = np.min(np.array(torch.cat([self.joints_input, self.joints_label], dim=1)))
        self.global_scene_max = np.max(np.array(self.scenes))
        self.global_scene_min = np.min(np.array(self.scenes))
    ...
    def __getitem__(self, index):
        ...
        return gazes, poses_input, poses_label, joints_input, joints_label, scene_points, motion_label

    def __len__(self):
        if self.train:
            return len(self.poses_path_list) * len(self.train_random_ori_list)
        else:
    ...
    def parse_data_info(self):
        for i, seq in enumerate(self.dataset_info['sequence_path']):
            if self.dataset_info['training'][i] != self.train: continue
    ...
    def _get_raw_item(self, index, random_ori):
        ...
        for f in range(self.output_seq_len):
            ...
            try:
                ...
                pose_path = os.path.join(self.dataroot, scene, seq, 'smplx_local', '{}.pkl'.format(end_frame))
                pose_data = pickle.load(open(pose_path, 'rb'))
        ...
        scene_points = self.scenes_raw[scene].clone() # this is must to clone it, or you can deepcopy it as well.
        ...
        scene_points = (random_rotation @ scene_points.numpy().T).T
    ...
    def _load_raw_scene(self, sample_points=32768):
        for i, scene_name in enumerate(self.dataset_info['scene']):
            if self.scenes_raw.get(scene_name, None) is None:
                ...
                scene_ply = trimesh.load(os.path.join(self.dataroot, scene_name, 'scene_obj', 'scene_downsampled.ply'))
    ...
    def _latent_to_joints(self, poses_full, vposer, body_model):
        joints_full = []
        if self.train:
            for i in range(len(self.poses_path_list)*len(self.train_random_ori_list)):
            ...
            for i in range(len(self.poses_path_list)):
                poses_sample = poses_full[i]
                pose = {}
                body_pose = vposer.decode(poses_sample[:, 6:], output_type='aa')
                body_pose = body_pose.view(body_pose.shape[0], -1)  # bs * 21 * 3 -> bs * 63
    ...
    def preprocess_dataset(self, folder, split):
        ...
        vposer, _ = load_vposer('/home/linyi/Code/SIF3D/vposer_v1_0', vp_model='snapshot')
        ...
        for i in range(len(self.poses_path_list)):
            if self.train:
                for ori_i in range(0,len(self.train_random_ori_list)):
                    random_ori = self.train_random_ori_list[ori_i]
                    gaze, pose_input, pose_label, scene_points, motion_label = self._get_raw_item(i, random_ori)
                ...
                gaze, pose_input, pose_label, scene_points, motion_label = self._get_raw_item(i, 0)
                gaze_full.append(gaze)
                pose_input_full.append(pose_input)
                pose_label_full.append(pose_label)
                scene_points_full.append(scene_points)
        ...
        if isinstance(scene_points_full[0], np.ndarray):
            scene_points_full = [torch.from_numpy(scene_points_full[i]) for i in range(len(scene_points_full))]

...
class EgoEvalDataset(data.Dataset):
    def __init__(self, config, train=False):
        ...
        self.dataset_info = pd.read_csv(os.path.join(self.dataroot, 'dataset_with_seg_test.csv'))
        ...
        self.scene_instances = torch.load(os.path.join(self.dataroot, folder, split, "scene_instances.pth"))        
    ...
    def __getitem__(self, index):  
        ...
        scene_instances = self.scene_instances[index]
        return gazes, poses_input, poses_label, joints_input, joints_label, scene_points, motion_label, index, scene_instances
    ...
    def _get_raw_item(self, index):
        ...
        scene_points = self.scenes_raw[scene].clone()
        ...
        scene_instances = self.scenes_seg[scene]
        for scene_instance in  scene_instances.values():
            scene_instance = scene_instance.copy()
            scene_instance *= 1 / scale
            scene_instance = (transform_norm[:3, :3] @ scene_instance.T + transform_norm[:3, 3:]).T
        
        return gazes, poses_input, poses_label, scene_points, motion_label, scene_instances
    ...
    def _load_raw_scene(self, sample_points=32768):
        for i, scene_name in enumerate(self.dataset_info['scene']):
            if self.scenes_raw.get(scene_name, None) is None:
                ...
                scene_file = scene_name + '.ply'
                scene_ply = trimesh.load(os.path.join('.../results_cgimo/ply_instance', scene_file))
    ...
    def _load_seg_scene(self):
        for i, scene_name in enumerate(self.dataset_info['scene']):
            if self.scenes_seg.get(scene_name, None) is None:
                print('loading scene of {}'.format(scene_name))
                scene_files = sorted([f for f in os.listdir(".../results_cgimo/ply_each_instance") if f.startswith(scene_name) and f.endswith('.ply')])
                scene_instances = {}
                for idx, filename in enumerate(scene_files, start=1):
                    file_path = os.path.join(".../results_cgimo/ply_each_instance", filename)
                    ply_data = trimesh.load(file_path)
                    ply_data = np.asarray(ply_data.vertices)
                    scene_instances[f"instance_{idx}"] = ply_data
                    print(f'loading scene instance {idx}')
                
                self.scenes_seg[scene_name] = scene_instances
    ...
    def preprocess_dataset(self, folder, split):
        ...
        print('loading poses')
        for i in range(self.__len__()):
            gaze, pose_input, pose_label, scene_points, motion_label, scene_instances = self._get_raw_item(i)
            ...
            scene_instance_full.append(scene_instances)
        ...
        torch.save(scene_instance_full, os.path.join(data_dir, "scene_instances.pth"))


class PosePromptDataset(data.Dataset):
    def __init__(self):
        self.dataset_path = "..."
        self.data = np.load(self.dataset_path, allow_pickle=True)
        self.data= self.data.item()
    
    def get_interaction_poses(self, items):
        poses_list = []
        for item in items:
            poses_list.append([self.data['motion'][item], self.data['text'][item]])
        poses_full = np.array(poses_list, dtype=object)
        return poses_full